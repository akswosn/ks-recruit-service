package com.forlks.ksrecruitservice.common.component

import com.forlks.ksrecruitservice.common.dto.JwtPayloadDto
import com.forlks.ksrecruitservice.common.exception.KsAPIException
...
//
    ...
    fun getMemberId(token: String?) = try {
        log.info("#### ${Jwts.parser().verifyWith(key).build()
                .parseSignedClaims(token).payload}")
        val result = Jwts.parser().verifyWith(key).build()
                .parseSignedClaims(token).payload.get("memberId", Integer::class.java)
        result.toLong()
    } catch (e: Exception){
